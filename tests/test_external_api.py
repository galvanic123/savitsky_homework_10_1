...
from src.external_api import currency_conversion_1
...
@patch("requests.get")
def test_currency_conversion(mock_get):  # type: ignore[no-untyped-def]
    mock_response = Mock()
    mock_response.text = json.dumps({"result": 2})
    mock_get.return_value = mock_response
    assert currency_conversion_1("USD", 1) == 2
...
@patch("requests.get")
def test_currency_conversion_flout(mock_get):  # type: ignore[no-untyped-def]
    mock_response = Mock()
    mock_response.text = json.dumps({"result": 3.5})
    mock_get.return_value = mock_response
    assert currency_conversion_1("USD", 1) == 3.5
    mock_get.assert_called_once_with(
        "https://api.apilayer.com/exchangerates_data/convert?to=RUB&from=USD&amount=1", headers={"apikey": API_KEY}
    )
...
def test_api_failure(mock_get: MagicMock) -> None:
    ...
    mock_get.return_value = mock_response
    assert currency_conversion_1("USD", 1) == 0.0
